...
TcpConnection::TcpConnection(
    tinyrpc::TcpClient* tcp_cli, tinyrpc::Reactor* reactor, int fd, int buff_size, NetAddress::ptr peer_addr):
...
void TcpConnection::input()
{
    ...
    while (!read_all) {
        ...
        if (rt <= 0) {
            DebugLog << "rt <= 0";
            ErrorLog << "read empty while occur read event, because of peer close, fd= " << m_fd
...
void TcpConnection::output()
{
    ...
    while (true) {
        ...
        DebugLog << "recycle write index =" << m_write_buffer->writeIndex()
                 << ", read_index =" << m_write_buffer->readIndex() << "readable = " << m_write_buffer->readAble();
