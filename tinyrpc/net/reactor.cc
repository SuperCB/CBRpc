                ...
                if (one_event.data.fd == m_wake_fd && (one_event.events & READ)) {
                    ...
                    if (ptr != nullptr) {
                        ...
                        if ((!(one_event.events & EPOLLIN)) && (!(one_event.events & EPOLLOUT))) {
                            ...
                            if (ptr->getCoroutine()) {
                                // the first one coroutine when epoll_wait back, just directly resume by this thread,
                                // not add to global CoroutineTaskQueue because every operate CoroutineTaskQueue should
                                // add mutex lock
                                if (!first_coroutine) {
                                ...
                                if (m_reactor_type == SubReactor) {
                                    ...
                                    // main reactor, just resume this coroutine. it is accept coroutine. and Main
                                    // Reactor only have this coroutine
                                    tinyrpc::Coroutine::Resume(ptr->getCoroutine());
                                    if (first_coroutine) {
                                        first_coroutine = NULL;
                                    }
