...
const char* TinyPbCodeC::encodePbData(TinyPbStruct* data, int& len)
{
    ...
    int32_t pk_len = 2 * sizeof(char) + 6 * sizeof(int32_t) + data->pb_data.length() + data->service_full_name.length()
...
void TinyPbCodeC::decode(TcpBuffer* buf, AbstractData* data)
{
    ...
    DebugLog << "m_read_buffer size=" << buf->getBufferVector().size() << "rd=" << buf->readIndex()
             << "wd=" << buf->writeIndex();
    ...
    int pb_data_len = pb_struct->pk_len - pb_struct->service_name_len - pb_struct->msg_req_len - pb_struct->err_info_len
